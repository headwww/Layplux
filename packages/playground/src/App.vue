...
// ═══ 亮暗切换 ────────────────────────────────────────────────────────
const schemeIcons: Record<string, string> = { light: '☀️', dark: '🌙', system: '💻' };
...
const ThemeSwitcher = defineComponent({
  setup: () => () =>
    h(
      ...
      schemeIcons[skeleton.theme.value],
    ),
});
...
skeleton.add({
  ...
  content: h('div', { style: { display: 'flex', gap: '4px', alignItems: 'center' } }, [
    h('button', { class: 'ide-btn' }, [h(PlayCircleOutlined), ' Run']),
  ...
  content: h('div', { class: 'panel-content' }, [
    h(
      ...
      ['App.vue', 'setup()', 'toggleLocale()', 'skeleton', 'ProjectPanel'].map((n) =>
        h('div', { class: 'ide-hover', style: { padding: '2px 0', cursor: 'pointer' } }, '🔹 ' + n),
      ),
    ),
  ]),
});
...
skeleton.add({
  ...
  index: 2,
  content: h('div', { class: 'panel-content' }, [
    h('div', { style: { padding: 8, fontSize: 11 } }, 'No bookmarks yet'),
  ]),
});
